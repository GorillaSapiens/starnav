...
# pick some random star...
my $pick = $stars[int(rand(scalar @stars))];

# find all stars within 15 degrees
my @picks;

my ($ra0, $de0, $bright0) = split / /, $stars{$stars[$pick]};
for (my $i = 0; $i <= $#stars; $i++) {
   my ($ra1, $de1, $bright1) = split / /, $stars{$stars[$i]};
   my $sep = angsep_microdeg($ra1, $de1, $ra0, $de0);
...
my $n = 0;
foreach my $item (@picks) {
   print $item . " " . $stars{$item} . "\n";
   $n++;
   if ($n == 5) {
      last;
   }
...
print "==\n";
for (my $i = 0; $i < 5; $i++) {
   my ($ra1, $de1, $bright1) = split / /, $stars{$picks[$i]};
   for (my $j = $i + 1; $j < 5; $j++) {
